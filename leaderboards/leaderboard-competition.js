(function(window, document, undefined) {
	...
	var cLabs = {
		api: {
			url: apiURL,
			leaderBoardQuery: "/api/v1/:space/contests/:contestId/leaderboard",
			structureQuery: "/api/v1/:space/competitions/:competitionId",
			contestQuery: "/api/v1/:space/contests/:id",
			preLoader: apiURL + "/assets/images/preloaders/pre-loader.gif",
			preLoaderDark: apiURL + "/assets/images/preloaders/pre-loader-dark.gif",
			css: {
				standardTheme: apiURL + "/assets/css/widgets/leaderboard.css",
				action: apiURL + "/assets/css/widgets/competition-leaderboard-dark.css"
			}
		},
	...
	var leaderBoardDefaultClass = ".cl-competition-leaderboard-widget";
	var leaderBoard = function( options ){
		...
		this.settings = {
			...
			reload: true,
			contest: null,
			container: leaderBoardDefaultClass,
			roundCount: 6, // slight visual restriction
			limit: 500,
			apiURL: apiURL,
			api: cLabs.api
		};
		...
		this.structureQuery = this.settings.api.url + this.settings.api.structureQuery;
		this.url = "";
		this.timeout;
		...
		this.leaderboardWrapper = function( containerString, callback ){
			var _this = this;
			
			objectIterator(query(containerString), function(widget) {
				if ( widget !== null && query(widget, ".cl-leaderboard-container") === null ) {
					_this.leaderboardWrapperLayout(widget);
					
					
					widget.addEventListener("click", function(event){
						if( event.target.nodeName === "LI" && hasClass(event.target, "cl-contest") && !hasClass(event.target, "cl-active-contest") ){
							removeClass(query(widget, ".cl-active-contest"), "cl-active-contest");
							addClass(event.target, "cl-active-contest");
							
							_this.contestId = getDataSet(event.target, "id");
							_this.settings.contest = null;
							_this.timeoutTime = 1000;
							_this.getData();
						}else if( event.target.nodeName === "LI" && hasClass(event.target, "cl-round") && !hasClass(event.target, "cl-active-round") ){
							removeClass(query(widget, ".cl-active-round"), "cl-active-round");
							addClass(event.target, "cl-active-round");
							
							objectIterator( query(widget, ".cl-leaderboard-contest-navigation .cl-contest"), function(contest, key, count){
								if(contest !== null){
									contest.style.display = "none";
									removeClass(contest, "cl-active-contest");
									addClass(contest, "hidden");
								}
							});
							
							var round = getDataSet(event.target, "round");
							objectIterator( query(widget, ".cl-leaderboard-contest-navigation .cl-round-" + round), function(contest, key, count){
								if(contest !== null){
									contest.style.display = "block";
									removeClass(contest, "hidden");
									
									if(count === 0){
										addClass(contest, "cl-active-contest");
										_this.contestId = getDataSet(contest, "id");
										_this.settings.contest = null;
									}
								}
							});
		...
		this.leaderboardWrapperLayout = function( container ){
			var _this = this,
				wrapper = document.createElement("div"),
				roundNavigation = document.createElement("ul"),
				contestNavigation = document.createElement("ul"),
				leaderboardContainer = document.createElement("div");
			
			
			wrapper.setAttribute("class", "cl-leaderboard-container");
			contestNavigation.setAttribute("class", "cl-leaderboard-contest-navigation");
			roundNavigation.setAttribute("class", "cl-leaderboard-round-navigation");
			leaderboardContainer.setAttribute("class", "cl-leaderboard-widget-inner-wrapper");
			
			var activeContest = false,
				roundCount = 0,
				contestCount = {};
			mapObject(_this.dataStructure.data.contests, function(contest, key, count){
				var contestLi = document.createElement("li"),
					activeClass = ( contest.round === 1 && !activeContest ) ? " cl-active-contest" : "",
					hiddenClass = ( contest.round !== 1 ) ? " hidden" : "";
				
				if( query(roundNavigation, ".cl-round-" + contest.round) === null && roundCount < _this.settings.roundCount ){
					var roundLi = document.createElement("li");
					
					roundLi.setAttribute("class", "cl-round cl-round-" + contest.round + ((contest.round === 1 && !activeContest) ? " cl-active-round" : ""));
					roundLi.innerHTML = "Round " + contest.round;
					
					setDataSet(roundLi, "round", contest.round);
					
					roundNavigation.appendChild(roundLi);
					
					roundCount++;
				}
				
				if( typeof contestCount["round-" + contest.round] === "undefined" || (typeof contestCount["round-" + contest.round] !== "undefined" && contestCount["round-" + contest.round].length < 8) ) {
					
					contestLi.setAttribute("class", "cl-contest " + "cl-round-" + contest.round + activeClass + hiddenClass);
					contestLi.innerHTML = contest.label;
					setDataSet(contestLi, "id", contest.id);
					
					contestNavigation.appendChild(contestLi);
					
					if (contest.round === 1 && !activeContest) {
						_this.contestId = contest.id;
						activeContest = true;
					}
					
					if (typeof contestCount["round-" + contest.round] === "undefined") {
						contestCount["round-" + contest.round] = [];
					}
					
				}
				
				contestCount["round-" + contest.round].push(contest.id);
			});
			...
			wrapper.appendChild(roundNavigation);
			wrapper.appendChild(contestNavigation);
			wrapper.appendChild(leaderboardContainer);
			
			return container.appendChild(wrapper);
		};
		
		this.layout = function( data , container){
			var _this = this;
			
			objectIterator(container, function(widget){
				if(widget !== null){
					...
					if( (typeof data.data !== "undefined" && data.data.length === 0) || typeof data.data === "undefined" || objectCount(query(widget, ".list .cl-result-list-item")) === 0 ){
						var li = document.createElement("li");
		...
		this.getStructure = function( callback ){
			...
			if( typeof _this.settings.competitionId !== "undefined" ) {
				
				if( _this.dataStructure === null ) {
					_this.url = _this.structureQuery.replace(":competitionId", _this.settings.competitionId).replace(":space", _this.settings.spaceName);
...
;							if (xhr.status === 200 && _this.dataString !== data) {
								...
								_this.dataStructure = json;
								
								callback();
							}
						},
						error: function(e){
							if(_this.settings.debug) console.log("[Comp L] Error: " + e);
						}
					};
		...
		this.destroy = function(){
			...
			obj._activeInstance = undefined;
			query(_this.settings.container).innerHTML = "";
			
			delete this;
			delete obj["_activeInstance"];
		};
		...
		this.getData = function( forceRefresh ){
			...
			_this.url = _this.leaderBoardQuery.replace(":space", _this.settings.spaceName).replace(":competitionId", _this.settings.competitionId).replace(":contestId", _this.contestId);
			
			if( !stringContains(_this.url, "_limit") ){
				params.push("_limit=" + _this.settings.limit)
			}
			
			if( typeof _this.settings.memberId === "string" && _this.settings.memberId.length > 0 ){
				params.push("memberId=" + _this.settings.memberId)
			}
			
			_this.url = _this.url + ( stringContains(_this.url, "?") ? "" : "?" ) + params.join("&");
		...
		var _getData = function(_this, forceRefresh){
			
			if ((typeof _this.settings.runOnLoad === "undefined" || (typeof _this.settings.runOnLoad === "boolean" && _this.settings.runOnLoad)) && _this.contestId !== null && _this.settings.spaceName !== null) {
				...
				var dataObj = {
					...
					success: function (data, dataObject, xhr) {
						...
						if (xhr.status === 200 && _this.dataString !== data) {
							...
							_this.layout(json, query(_this.settings.container + " .cl-leaderboard-widget-inner-wrapper"));
				...
				_this.ajaxDataGet.abort().getData(dataObj);
			}
		};
		...
		this.getContestData = function( callback ){
			...
			var url = _this.contestQuery.replace(":space", _this.settings.spaceName).replace(":id", _this.contestId);
		...
		this.refresh = function( forceRefresh ){
			...
			_this.getStructure(function() {
				_this.leaderboardWrapper(_this.settings.container, function(){
		...
		this.createStyleSheet = function(widget){
			...
			if (_this.settings.font !== null) {
				styleSheetContent += _this.settings.container + " .cl-leaderboard-container{font-family: '" + _this.settings.font + "';}";
			}
			
			if (_this.settings.fontColor !== null) {
				if( _this.settings.theme === "action" ){
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container *{color:' + _this.settings.fontColor + ';}';
				}else {
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container{color:' + _this.settings.fontColor + ';}';
				}
			}
			...
			if (_this.settings.backgroundColor !== null) {
				if( _this.settings.theme === "action" ){
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container .list{background-color:' + _this.settings.backgroundColor + ';}';
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container .cl-right.cl-points::before{border-left-color:' + _this.settings.backgroundColor + ';}';
					...
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container .cl-round{background-color:' + _this.settings.backgroundColor + ';}';
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container .cl-contest{background-color:' + _this.settings.backgroundColor + ';}';
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container{color:' + _this.settings.fontColor + ';}';
				}
			}
			...
			if (_this.settings.pointsBackgroundColor !== null) {
				if( _this.settings.theme === "action" ){
					styleSheetContent += _this.settings.container + ' .cl-leaderboard-container .cl-points{background-color:' + _this.settings.pointsBackgroundColor + ';}';
				}
			}
			
			style.appendChild(document.createTextNode(styleSheetContent));
			
			query(widget, "img").src = ( ( _this.settings.theme === "action" ) ? _this.settings.api.preLoaderDark : _this.settings.api.preLoader);
		...
		this.init = function() {
			...
			if( typeof _this.settings !== "undefined" ) {
				...
				if( obj !== null && typeof obj._activeInstance === "undefined" ) {
					...
					if(_this.settings.debug) console.warn("[CompetitionLabs Leaderboard]: container not found ");
				}else{
					if(_this.settings.debug) console.warn("[CompetitionLabs Leaderboard]: Instance already exists for this obj => " + leaderBoardDefaultClass);
					_this.refresh( true );
				}
			}
		};
	...
	if(typeof window._clCompOptions !== "undefined"){
		mapObject(window._clCompOptions, function(opt, key){
			if(typeof window._clCompOptions[key].obj === "undefined") {
				window._clCompOptions[key].obj = new leaderBoard(opt);
			}
		});
	}
	
	if(typeof window._clCompLeaderBoard === "undefined") {
		window._clCompLeaderBoard = leaderBoard;
	}else{
		console.warn("window._clCompLeaderBoard is already defined");
	}
